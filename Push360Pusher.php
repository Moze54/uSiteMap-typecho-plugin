 ...
 * 负责处理与360站长平台的交互
 */
class uSitemap_Push360Pusher extends uSitemap_SearchEnginePusher
{
    /**
     * API推送接口地址
     */
    const API_PUSH_URL = 'http://zhanzhang.so.com/push';
    ...
    public function pushUrl($url)
    {
        if (!$this->isConfigured()) {
            return $this->error('360推送配置不完整，请检查站点和Token设置');
        }
    ...
    private function buildApiUrl()
    {
        ...
        return self::API_PUSH_URL . '?site=' . urlencode($site) . '&token=' . urlencode($token);
    }

    /**
     * 发送HTTP请求（重写以适配360的特殊格式）
     *
    ...
    {
        $config = array(
            'site' => isset($pluginOptions->push360Site) ? $pluginOptions->push360Site : '',
            'token' => isset($pluginOptions->push360Token) ? $pluginOptions->push360Token : '',
            'type' => isset($pluginOptions->push360PushType) ? $pluginOptions->push360PushType : 'api',
            'triggers' => isset($pluginOptions->push360PushTrigger) ? $pluginOptions->push360PushTrigger : array()
        );
