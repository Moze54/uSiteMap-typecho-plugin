...
class uSiteMap_Plugin implements Typecho_Plugin_Interface
{
    ...
    {
        Helper::addRoute('sitemap-api', '/sitemap-api', 'uSiteMap_API', 'action');
        Helper::addRoute('sitemap', '/sitemap.xml', 'uSiteMap_XML', 'action');
        Helper::addPanel(1, 'uSiteMap/Push.php', '推送百度', '推送百度', 'administrator');
        require_once("Action.php"); //引入
        update('activate', 'web'); //激活xml
        Typecho_Plugin::factory('Widget_Contents_Post_Edit')->finishPublish = array('uSiteMap_Plugin', 'auto');
        Typecho_Plugin::factory('Widget_Contents_Page_Edit')->finishPublish = array('uSiteMap_Plugin', 'auto');
    }
    ...
    public static function deactivate()
    {
        ...
        Helper::removePanel(1, 'uSiteMap/Push.php');
    ...
    public static function config(Typecho_Widget_Helper_Form $form)
    {
        ...
        try {
            if (Helper::options()->plugin('uSiteMap')->api_token == null) {
                $randString = Typecho_Common::randString(32);
            } else {
                $randString = Helper::options()->plugin('uSiteMap')->api_token;
            }
        } catch (Exception $e) {
            $randString = Typecho_Common::randString(32);
        }
        ...
        $Btn->input->setAttribute('formaction', Typecho_Common::url('/options-plugin.php?config=uSiteMap&action=update_sitemap', Helper::options()->adminUrl));
        $form->addItem($Btn);
    }
    /**
     * 更新 sitemap
    ...
    public static function update_sitemap()
    {
        ...
        header("location:" . Typecho_Common::url('/options-plugin.php?config=uSiteMap', Helper::options()->adminUrl));
    }
    /**
     * 插件实现方法
     * 
    ...
    public static function auto($contents, $class)
    {
        ...
        //检查自动推送文章开关
        if (Helper::options()->plugin('uSiteMap')->AutoPush == 1) {
            $api = Helper::options()->plugin('uSiteMap')->baidu_url;
            $ch = curl_init();
            $options =  array(
                CURLOPT_URL => $api,
                CURLOPT_POST => true,
            ...
            if (Helper::options()->plugin('uSiteMap')->baidu_url == null) {
                $log = '【' . $time . '】' . 'auto' . '推送失败，未填写百度推送接口 URL';
            } else {
                $api = Helper::options()->plugin('uSiteMap')->baidu_url;
            }
            if (Helper::options()->plugin('uSiteMap')->PluginLog == 1) {
                file_put_contents(__TYPECHO_ROOT_DIR__ . __TYPECHO_PLUGIN_DIR__ . '/uSiteMap/log.txt', $log, FILE_APPEND);
            }
        }

        //检查自动更新Sitemap开关
        if (Helper::options()->plugin('uSiteMap')->AutoSitemap == 1) {
            //获取时间
            $time = date('Y-m-d H:i:s', time());
            //更新sitemap
            require_once("Action.php");
